...
class Trajectory2():
    ...
    def Jac(self, q):
        ...
        R0, pos0, _, _ = self.chain0.fkin(q)
        R1, pos1, _, _ = self.chain1.fkin(q)
        R2, pos2, _, _ = self.chain2.fkin(q)
        R3, pos3, _, _ = self.chain3.fkin(q)
    ...
    def evaluate(self, t, dt):
        ...
        R0, pos0, _, _ = self.chain0.fkin(qdlast)
        R1, pos1, _, _ = self.chain1.fkin(qdlast)
        R2, pos2, _, _ = self.chain2.fkin(qdlast)
        R3, pos3, _, _ = self.chain3.fkin(qdlast)
        F0 = T_from_Rp(R0, pos0)
        F1 = T_from_Rp(R1, pos1)
        F2 = T_from_Rp(R2, pos2)
        F3 = T_from_Rp(R3, pos3)
